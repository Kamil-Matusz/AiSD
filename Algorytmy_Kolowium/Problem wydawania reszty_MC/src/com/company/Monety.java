...
public class Monety {
    ...
    private static final int[] LICZEBNOŚĆ = {1000, 1, 5, 0, 0, 0, 0, 0, 1}; // index 0 -> 5, so we have five 5zl, two 2zl, etc...
    ...
    private static void keepYourChangeMonteCarlo() {
        ...
        for(int i = 0; i < LICZBA_LOSOWAŃ; i++) {
            ...
            int[] aktualnaLiczebność = LICZEBNOŚĆ.clone();

            while ((r > 0 && liczbaWydanychMonet < LIMIT_WYDANYCH_MONET)) {
                int losowaMoneta = RANDOM.nextInt(MONETY.length);
                int monety = MONETY[losowaMoneta];
                int liczbMonet = aktualnaLiczebność[losowaMoneta];
                int najmniejszaLiczbaMonet = findLowest(aktualnaLiczebność);
                if(najmniejszaLiczbaMonet > r) {
                    break;
                }
                if(monety <= r && liczbMonet > 0) {
                    wydaneMonety[liczbaWydanychMonet] = monety;
                    aktualnaLiczebność[losowaMoneta]--;
                    r -= monety;
                    liczbaWydanychMonet++;
                }
            }
